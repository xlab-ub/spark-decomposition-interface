...
class NoopRobotBackend(RobotBackend):
    """Unitree Go2 dry-run backend: checks commands against the go2 vocabulary and logs them (no hardware)."""
    ...
    def check_simplified_syntax_validity(self, simplified_code: str) -> bool:
        ...
        try:
            ...
            print(f"[go2-noop] Invalid syntax: {e}")
            return False
        for node in ast.walk(parsed_code):
            ...
            if isinstance(func, ast.Attribute) and isinstance(func.value, ast.Name) and func.value.id == "self":
                if func.attr not in self.allowed_calls:
                    print(f"[go2-noop] Unknown command for go2: {func.attr}")
                    return False
            elif isinstance(func, ast.Name) and func.id == "range":
                continue
            else:
                print("[go2-noop] Function call does not start with 'self.'")
                return False
        return True

    def execute_simplified_syntax(self, simplified_code: str) -> None:
        print(f"[go2-noop] Would execute:\n{simplified_code}")
