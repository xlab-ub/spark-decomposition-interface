...
- "jump" means SPIN_JUMP.
- "dance" means FIRST_DANCE unless a specific dance is named.
...
STAND_UP, TILT_LEFT_SHOULDER, TILT_RIGHT_SHOULDER, TILT_HEAD_UP, TILT_HEAD_DOWN, TILT_HEAD_LEFT, TILT_HEAD_RIGHT, MOVE_FORWARD, MOVE_LEFT, MOVE_RIGHT, TURN_LEFT, TURN_RIGHT, SPIN_JUMP, LIFT, FIRST_DANCE, SECOND_DANCE
...
- Sequential actions: Example 8 (`TURN_RIGHT` then `TILT_HEAD_DOWN`)
- IF + REPEAT + extra action: Example 9 (`IF FAR`, `REPEAT 2 TIMES`, `FIRST_DANCE`)
...
### Example 3
User Instruction: spin jump when far

### High-Level Task ###
The robot needs to perform a spin jump when it is far from an object.
...
If the object is far, the robot will perform a spin jump.

### Logical Relations ###
The condition "far" determines when the robot should perform the spin jump.
The action "spin jump" is executed when the robot is far from the object.
...
If the condition is true, the robot performs the SPIN_JUMP action.
...
### Example 7
User Instruction: jump two times

### High-Level Task ###
The robot needs to jump twice.

### Pseudo Instruction ###
REPEAT_JUMP_TWICE

### Natural Language Plans ###
The robot will jump twice.

### Logical Relations ###
The action "jump" is repeated twice.
...
### Explanation ###
The REPEAT 2 TIMES loop ensures that the robot performs the SPIN_JUMP action twice, as described in the natural language plan.

---

### Example 8
User Instruction: let's turn right and move head down

### High-Level Task ###
The robot needs to turn right and tilt its head down.

### Pseudo Instruction ###
TURN_RIGHT_AND_TILT_HEAD_DOWN

### Natural Language Plans ###
The robot will turn right.
The robot will tilt its head down.

### Logical Relations ###
The action "turn right" precedes the action "tilt head down."

### Components for Pseudo Code ###
Control:
Condition:
Action: TURN_RIGHT, TILT_HEAD_DOWN
...
The TILT_HEAD_DOWN command corresponds to the second step in the plan, where the robot tilts its head down.
...
The FIRST_DANCE command corresponds to the action of dancing when the robot is not touching an object.

---

### Example 10
User Instruction: let's tilt right shoulder when not reaching, otherwise tilt left shoulder

### High-Level Task ###
The robot needs to tilt its right shoulder when not reaching for an object; otherwise, it should tilt its left shoulder.

### Pseudo Instruction ###
TILT_RIGHT_SHOULDER_IF_NOT_REACHING_ELSE_TILT_LEFT_SHOULDER

### Natural Language Plans ###
The robot will check if it is reaching for an object.
If the robot is not reaching, it will tilt its right shoulder.
Otherwise, it will tilt its left shoulder.

### Logical Relations ###
The condition "far" determines when the robot should tilt its right shoulder.
The action "tilt right shoulder" is performed when the robot is far from the object.
The action "tilt left shoulder" is performed when the robot is not far from the object.

### Components for Pseudo Code ###
Control: IF, ELSE
Condition: FAR
Action: TILT_RIGHT_SHOULDER, TILT_LEFT_SHOULDER
...
### Explanation ###
The IF FAR condition checks if the robot is far from an object.
If the condition is true, the robot tilts its right shoulder.
Otherwise, the robot tilts its left shoulder.
